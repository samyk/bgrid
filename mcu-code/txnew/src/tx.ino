...
//#define BAD_DATA do { pf("BAD\n"); while (Serial.available() && Serial.read() != '\n'); return; } while(0)

void printSerial()
{
  int sc, bytes = 0;
  while (bytes < MAX_SER_BYTES && (sc = Serial.read()) != -1)
    sermem[bytes++] = sc;

  if (bytes == 0) return;
  // print all of sermem in hex
  pf("b=%d ", bytes);
...
void readPix()
{
  ...
  // a[length]
  else if (startChar == 'A' || startChar == 'a') // A [bytes] <bytes of data to split and TX> \n
  {
